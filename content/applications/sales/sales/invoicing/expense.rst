...
clicing :guilabel:`Add a product`. Next, select a product from the drop-down menu.
...
Then, from the *Expenses* dashboard, click :guilabel:`New`. Clicking :guilabel:`New` reveals a blank
expenses form.
...
- :guilabel:`Travel & Accomodation`: any travel or accomodation costs related to project/order.
...
Next, designate if there is an :guilabel:`Included taxes` in the :guilabel:`Total`. If a
...
If the cost of this expense needs to be split, click :guilabel:`Split Expense`.

However, in this case, neither of those options are necessary, so we'll simply click
:guilabel:`Create Report` to lock in the expense report that was just configured.

Doing so reveals an :guilabel:`Expense Report Summary` for our expense.
...
approrpriate sales order that should be reinvoiced for the expense.
...
Payment` banner is in the upper-right corner of the invoice, signifying this invoice is complete and
paid for in full. Thus, completing the workflow.
